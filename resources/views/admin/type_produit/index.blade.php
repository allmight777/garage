        ...
        @else
            ...
            <td class="actions-cell">
                <div class="actions-buttons">
                    <a href="{{ route('type_produits.edit', $t) }}" class="btn-action btn-edit" data-tooltip="Modifier">
                        <i class="fas fa-edit"></i>
                    </a>

                    <form action="{{ route('type_produits.destroy', $t) }}" method="POST" class="delete-form">
                        @csrf @method('DELETE')
                        <button type="button" class="btn-action btn-delete delete-btn" data-name="{{ $t->nom }}"
                            data-tooltip="Supprimer">
                            <i class="fas fa-trash"></i>
                        </button>
                    </form>

                    <a href="#" class="btn-action btn-view view-btn" data-tooltip="Voir détails"
                        data-id="{{ $t->id }}" data-reference="{{ $t->reference }}"
                        data-nom="{{ $t->nom }}" data-description="{{ $t->description }}"
                        data-image="{{ $t->image ? asset('storage/type_produits/' . $t->image) : '' }}"
                        data-created_at="{{ $t->created_at }}" data-updated_at="{{ $t->updated_at }}">
                            ...
                            <div class="detail-percent">
                                ...
                                <div class="percent-bar">
                                    <div class="percent-fill"
                                        style="width: 25%; background-color: {{ getColor($loop->index) }}"></div>
                                </div>
                            </div>
                        </div>
                    @endforeach
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.toggle-btn').forEach(btn => {
                btn.addEventListener('click', function() {
                    document.querySelectorAll('.toggle-btn').forEach(b => b.classList.remove(
                        'active'));
